...
try {
  ...
  assert.equal(runtime.inputs.filter(x => x.terminal === 'term_test-A').map(x => x.text).join(''), '/model\x1b[B\r\x1b\r')
  ...
  console.log('PASS: two real xterm renderers, raw keys, independent routing, focus, scrollback, reconnect, maximize, authentication, and detach-only cleanup')
